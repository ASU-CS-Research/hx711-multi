...
class HX711:
    ...
    Args:
        dout_pins (int or [int]): Raspberry Pi GPIO pins where data from HX711 is received
        sck_pin (int): Raspberry Pi clock output pin where sck signal to HX711 is sent
        channel_a_gain (int): Optional, by default value 128
            Options (128 || 64)
        channel_select (str): Optional, by default 'A'
            Options ('A' || 'B')
        ...
        log_level (str or int): Optional, prints out info to console based on level of log
    ...
    def _prepare_to_read(self):
        ...
        for i in range(20):
            ...
            if all([adc.is_ready() for adc in self._adcs]):
                break
            else:
                # if not ready sleep for 10ms before next iteration
                sleep(0.01)
        ready = all([adc.ready for adc in self._adcs])
        if ready:
            self._logger.debug(
    ...
    def _read(self):
        ...
        operation:
            ...
            3) set channel gain following read by pulsing SCK to result in a total of 25, 26, or 27 SCK pulses
            for a read operation (see documentation)
    ...
    def read_raw(self, readings_to_average: int = 10):
        ...
        if not (1 <= readings_to_average <= 10000):
            raise ValueError(
                f'Parameter "readings_to_average" input to read_raw() is way too high... '
                f'Received: {readings_to_average}')

        adc: ADC
        # init each adc for a set of reads
        for adc in self._adcs:
            adc.init_set_of_reads()
        # perform reads
    ...
    def zero(self, readings_to_average: int = 30, retry_limit: int = 10):
        ...
        for adc in self._adcs:
            if adc.ready:
                self._logger.debug(
                    f'zeroing with {len(adc.reads_filtered)} data points')
                adc.zero_from_last_measurement()

    def set_weight_multiples(self, weight_multiples, adc_indices=None, dout_pins=None):
        """
        Sets the weight multiples for ADCs to be used when calculating weight
        Example: scale indicates value of 5000 for 1 gram on scale,
        weight_multiple = 5000 (to convert to weight in grams)

        Args:
            weight_multiples (float | list of floats): value(s) to be set on ADCs

            adc_indices (int | list of ints, optional): indices of ADC with reference
            to ordering of dout pins during initialization
        ...
        if dout_pins is not None:
            ...
            adcs = [adc for adc in self._adcs if adc.dout_pin in dout_pins]
        else:
    ...
    def get_offset(self):
        """
        Gets zero_offset of the all adcs:
        ...
        """
        adc: ADC
        offsets = [adc.zero_offset for adc in self._adcs]
        return offsets
...
class ADC:
    ...
    Attrs:
        ...
        _max_stdev (int):           max standard deviation value of raw reads
        (future todo: expose for user input? Does this vary per hardware?)
        _reads_filtered ([int])     filtered reads after removing failed reads and bad data points
        _max_number_of_stdev_from_med (float): maximum number of deviations from median
        (future todo: expose for user input?)
        ...
        _measurement (float):        mean value of raw reads after filtering
        _measurement_from_zero (float): measurement minus offset
        _weight (float):             measurement_from_zero divided by weight_multiple
    """
    ...
    def calculate_measurement(self):
        ...
        if not len(self._reads_filtered):
            ...
            self._measurement = self._reads_filtered[0]
            return True
        ...
        if self._read_stdev > self._max_stdev:
            ...
            # sometimes with a bad scale connection, the bit will come back ready out of chance
            # and the binary values are garbage data
            self._ready = False
            self._logger.warning(
                f'ADC (dout {self._dout_pin}) not ready, stdev from '
                f'median was over {self._max_stdev}: {self._read_stdev}')
            ...
            self._measurement = self._read_med
            return True
        _new_reads_filtered = []
        ...
        if not self._reads_filtered:
            # no values after filter, so return False to indicate no read value
            return False
        self._measurement = mean(self._reads_filtered)
        self._measurement_from_zero = self._measurement - self._zero_offset
        self._weight = self._measurement_from_zero / self._weight_multiple

        return True

    @property
    def dout_pin(self):
        return self._dout_pin

    @property
    def reads_filtered(self):
        return self._reads_filtered

    @property
    def measurement_from_zero(self):
        return self._measurement_from_zero
